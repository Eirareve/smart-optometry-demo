...
Phase 1 首页和统一 `DeviceAdapter` 抽象契约已完成。首页包含 DEMO MODE、设备状态、设备编号、连接状态、最后通信时间和“开始智能验光”入口；设备领域类型与连接、状态、检测、取消、结果读取接口已建立。当前未实现 Mock Device、真实设备通讯或检测流程，因此页面不会显示设备已连接，开始按钮也不会启动检测。
