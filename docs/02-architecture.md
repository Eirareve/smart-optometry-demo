...
本文描述 V0.1 网页 Demo 的目标架构。当前已完成 React 基础工程和首页 UI；Exam Service、DeviceAdapter、Mock Device、检测流程、结果页和报告页尚未实现。
...
当前已实现首页，包含 Demo 标识、设备状态卡和操作入口。首页只呈现明确的“设备未接入”静态状态，不调用或模拟设备能力。检测页、结果页、报告页和可选开发调试页仍为计划内容。
